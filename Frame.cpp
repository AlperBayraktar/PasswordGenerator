...
void Frame::GeneratePassword(wxCommandEvent& evt)
{
    ...
    if (chars == "")
    {
        wxMessageBox("En az bir seçenek işaretlenmelidir.", "Hata", wxOK | wxICON_ERROR);
        return;
    }
...
{
    wxMessageBox("Kopyalama işlemi başarısız,\n lütfen manuel olarak kopyalamayı deneyin.", "Hata", wxOK | wxICON_ERROR);
}
...
void Frame::CopyText(std::string text)
{
    ...
    else
    {
        ...
        wxMessageBox("Metin kopyalandı.", "Kopyalandı", wxOK | wxICON_INFORMATION);
    }
}
